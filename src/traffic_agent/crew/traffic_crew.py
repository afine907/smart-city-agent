...
from traffic_agent.llm.client import LLMClient, LLMConfig, LLMResponse
...
class DecisionCache:
    """Simple decision cache based on state similarity."""
    
    def __init__(self, max_size: int = 1000):
        self.cache: Dict[str, TrafficDecision] = {}
        self.max_size = max_size
    
    def _state_key(self, state: IntersectionState) -> str:
        """Generate cache key from state (coarse-grained)."""
        # Round queue lengths to reduce cache misses
        return (
            f"{state.queue_north // 5}_{state.queue_south // 5}_"
            f"{state.queue_east // 5}_{state.queue_west // 5}_"
    ...
    def set(self, state: IntersectionState, decision: TrafficDecision):
        if len(self.cache) >= self.max_size:
            # Remove oldest (simplified)
            self.cache.pop(next(iter(self.cache)))
        self.cache[self._state_key(state)] = decision
...
请做出信号灯决策（JSON格式）。"""
    ...
    def _coordinate(
        ...
        # Check for conflicts
        conflicts = self._detect_conflicts(decisions, states)
        ...
        for ix_id, decision in decisions.items():
            ...
            for nid in neighbors:
                if nid in decisions:
                    ...
                    # Conflict: both want green on same approaches
                    if (decision.phase == "NS_GREEN" and 
                        neighbor_decision.phase == "EW_GREEN"):
                        # Check if they share a road
                        if self._are_adjacent(ix_id, nid):
                            conflicts.append((ix_id, nid, "phase_conflict"))
        
        return conflicts
    
    def _are_adjacent(self, ix1: str, ix2: str) -> bool:
        """Check if two intersections are adjacent."""
        return ix2 in self.graph.get(ix1, []) or ix1 in self.graph.get(ix2, [])
